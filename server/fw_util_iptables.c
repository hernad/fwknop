    ...
    else /* Non-NAT request - this is the typical case. */
    {
       ...
       //if (strncasecmp(opts->config[CONF_ENABLE_DELETE_OLD_RULES], "Y", 1) == 0) {
          ...
          snprintf(cmd_buf, CMD_BUFSIZE-1,
             "/opt/apps/green/bin/iptables_delete_rules_by_ip.sh %s %s %s",
              opts->fw_config->fw_command, spadat->use_src_ip, in_chain->to_chain);
                  // "sh -c \"%s -L -v --line-numbers | grep ACCEPT.*%s.*_exp_ | awk '{ print $1 }' | sort -nr | xargs -n 1 %s -D %s\"",
                  //  opts->fw_config->fw_command, spadat->use_src_ip, opts->fw_config->fw_command, in_chain->to_chain);
          res = run_extcmd(cmd_buf, err_buf, CMD_BUFSIZE, WANT_STDERR, NO_TIMEOUT, &pid_status, opts);
          log_msg(LOG_DEBUG, "DELETE_OLD_RULES CMD: '%s' (res: %d, err: %s)",
              cmd_buf, res, err_buf);
        //}
